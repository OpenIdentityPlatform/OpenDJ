...
public class OpenDJTestListener extends TestListenerAdapter implements IReporter
{
  ...
  private static class TestClassResults
  {
    ...
    private final LinkedHashMap<ITestNGMethod, TestMethodResults> _methods =
      new LinkedHashMap<ITestNGMethod, TestMethodResults>();
   ...
   */
  private static String threadStacksToString()
  {
    ...
    // logical
    // order.
    final TreeMap<Long, Map.Entry<Thread, StackTraceElement[]>> orderedStacks =
  ...
  private final Set<Class<?>> _checkedForTypeAndAnnotations =
    new HashSet<Class<?>>();

  private final LinkedHashSet<Class<?>> _classesWithTestsRunInterleaved =
    new LinkedHashSet<Class<?>>();

  private Object _lastTestObject = null;

  private final IdentityHashMap<Object, Object> _previousTestObjects =
  ...
  private final LinkedHashMap<IClass, TestClassResults> _classResults =
    new LinkedHashMap<IClass, TestClassResults>();
  ...
  /**
   * Creates a new OpenDJ test listener.
   *
   * @throws Exception
  ...
  private void addTestResult(final ITestResult result)
  {
    ...
    for (final Object testInstance : testInstances)
    {
      ...
      {
        final OpenDJTestCase openDSTestCase = (OpenDJTestCase) testInstance;
        ...
        else
        {
          ...
          // everything that we need to print the result (e.g. the Schema for an
          // Entry or DN), so go ahead and convert it to a String now.
  ...
  private void enforceMethodHasAnnotation(final ITestResult tr)
  {
    ...
    if ((testAnnotation == null) && (dataProviderAnnotation == null))
    {
      final String errorMessage = "The test method "
          ...
          + "However, TestNG assumes it is a test method because it's a "
          + "public method in a class with a class-level @Test annotation.  "
          + "You can remove this warning by either marking the method with "
          + "@Test or by making it non-public.";
...
//          + "must include sequential=true to ensure that tests for a single "
//          + "class are run together.";
//      System.err.println("\n\nERROR: " + errorMessage + "\n\n");
//      throw new RuntimeException(errorMessage);
//    }
  }
