...
public class Storage implements org.opends.server.backends.pluggable.spi.Storage, ConfigurationChangeListener<CASBackendCfg>{
	...
	public void open(AccessMode accessMode) throws Exception {
		...
		if (AccessMode.READ_WRITE.equals(accessMode)) {
			try {
				PreparedStatement ps = prepared.get("CREATE KEYSPACE IF NOT EXISTS "+getKeyspaceName()+" WITH replication = {'class': 'SimpleStrategy', 'replication_factor': '1'};");
				execute(ps.bind().setExecutionProfileName(profile));
			} catch (Exception e) {
	...
	public void removeStorageFiles() throws StorageRuntimeException {
		...
		try {
			PreparedStatement ps = prepared.get("TRUNCATE TABLE "+getTableName()+";");
			execute(ps.bind().setExecutionProfileName(profile));
		} catch (Exception e) {
			throw new StorageRuntimeException(e);
		}
	...
	private final class TransactionImpl implements ReadableTransaction,WriteableTransaction {
		...
		public void openTree(TreeName name, boolean createOnDemand) {
			if (createOnDemand) {
				try
				{
					PreparedStatement ps = prepared.get("CREATE TABLE IF NOT EXISTS " + getTableName() + " (baseDN text,indexId text,key blob,value blob,PRIMARY KEY ((baseDN,indexId),key));");
					execute(ps.bind().setExecutionProfileName(profile));
				} catch (Exception e) {
					throw new StorageRuntimeException(e);
				}
		...
		public ByteString read(TreeName treeName, ByteSequence key) {
			try {
				PreparedStatement ps = prepared.get("SELECT value FROM "+getTableName()+" WHERE baseDN=:baseDN and indexId=:indexId and key=:key");
				final Row row = execute(ps
					.bind()
		...
		public long getRecordCount(TreeName treeName) {
			try {
				PreparedStatement ps = prepared.get("SELECT count(*) FROM "+getTableName()+" WHERE baseDN=:baseDN and indexId=:indexId");
				return execute(ps
					.bind()
					.setString("baseDN", treeName.getBaseDN()).setString("indexId", treeName.getIndexId())
				).one().getLong(0);
		...
		public void deleteTree(TreeName treeName) {
			...
			try {
				PreparedStatement ps = prepared.get("DELETE FROM "+getTableName()+" WHERE baseDN=:baseDN and indexId=:indexId");
				execute(ps
					.bind()
					.setString("baseDN", treeName.getBaseDN()).setString("indexId", treeName.getIndexId())
				);
		...
		public void put(TreeName treeName, ByteSequence key, ByteSequence value) {
			checkReadOnly();
			try {
				PreparedStatement ps = prepared.get("INSERT INTO "+getTableName()+" (baseDN,indexId,key,value) VALUES (:baseDN,:indexId,:key,:value)");
				execute(ps
					.bind()
					.setString("baseDN", treeName.getBaseDN()).setString("indexId", treeName.getIndexId())
					.setByteBuffer("key", ByteBuffer.wrap(key.toByteArray()))
		...
		public boolean delete(TreeName treeName, ByteSequence key) {
			checkReadOnly();
			try {
				PreparedStatement ps = prepared.get("DELETE FROM "+getTableName()+" WHERE baseDN=:baseDN and indexId=:indexId and key=:key");
				execute(ps
					.bind()
					.setString("baseDN", treeName.getBaseDN()).setString("indexId", treeName.getIndexId())
					.setByteBuffer("key", ByteBuffer.wrap(key.toByteArray()))
	...
	private final class CursorImpl implements Cursor<ByteString, ByteString> {
		...
		ResultSet full(){
			try {
				PreparedStatement ps = prepared.get("SELECT key,value FROM "+getTableName()+" WHERE baseDN=:baseDN and indexId=:indexId ORDER BY key");
				return execute(ps
					.bind()
					.setString("baseDN", treeName.getBaseDN()).setString("indexId", treeName.getIndexId())
				);
