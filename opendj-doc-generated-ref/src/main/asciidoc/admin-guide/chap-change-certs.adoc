...
Each `keytool` command applies to a single server, and is run while that server is stopped. A running server keeps a copy of the `ads-truststore` in memory and can overwrite changes made to the file.
...
repl-cert, Sep 1, 2026, PrivateKeyEntry,
Certificate fingerprint (SHA-256): 3F:...:1C
----
