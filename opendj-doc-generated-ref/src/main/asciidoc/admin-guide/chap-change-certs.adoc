...
----
+
The `server-cert` key pair is copied into the `ads-truststore`, the certificates which issued it are trusted there, and the `ssl-cert-nickname` property of the crypto manager is set to `server-cert`. The same applies to the `--useJavaKeystore`, `--useJCEKS` and `--useBcfksKeystore` options. A certificate generated by the installer and a key held in a PKCS#11 token cannot be used, the first because it would be self-signed like the one it replaces, the second because its private key cannot be exported.
