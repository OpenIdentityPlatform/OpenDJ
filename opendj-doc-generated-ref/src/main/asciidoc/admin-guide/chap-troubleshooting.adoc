...
[27/Jun/2011:14:37:48 +0200] category=SYNC severity=WARNING msgID=105
 ...
 the information severity, which the error log does not publish by default
 (0 since the previous warning).
...
Every connection reaching the replication port goes through the first two of these, and through the failed handshake above, so each of the three is logged as a warning at most once every five minutes, and each warning reports how many failures it stands for. The failures they suppress are still recorded, with the `information` severity, which the error log does not publish by default: add `information` to the `default-severity` property of the error log publisher to see them. They do not go to the debug log.
