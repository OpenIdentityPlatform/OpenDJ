...
Note that only one access logger, the File-Based Access Logger, is enabled in the default configuration. The Json File-Based Access Logger records the same operations in JSON format and is disabled by default: each enabled access logger adds its own log messages for every client request, so running several access loggers multiplies the logging cost on the operation processing path.
